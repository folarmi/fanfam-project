    ...
    try {
      ...
      commentSubRef.current = client.subscribe(commentTopic, (message) => {
        try {
          ...
          // Previously gated on payload.event === "LIVE_COMMENT" — but the
          // /topic/live/{sessionId}/comment destination is already scoped
          // to comments by definition (same reasoning we applied to the
          // end-topic fix). If a creator-originated comment doesn't carry
          // that exact event string (which, given everything else this
          // backend has done differently by sender role, wouldn't be a
          // surprise), the old gate silently dropped it for every viewer.
          // Trusting the topic instead and just requiring actual message
          // content.

          // Confirmed from a real payload capture: the backend identifies
          // the sender with `sender` (an email string) — not `user`,
          // `userId`, or `username` as originally assumed — and it does
          // not include `sessionID` or echo back `clientMessageId`.
          const comment: LiveComment = {
            id: payload.id || payload.commentId || Date.now(),
            sessionID: payload.sessionID || sessionId,
            message: payload.message || payload.text || "",
            user:
              payload.sender || payload.user || payload.username || "Anonymous",
            userId: payload.userId || payload.usid,
            username:
              payload.username ||
              payload.displayName ||
          ...
          (comment as any).sender = payload.sender;
          (comment as any).clientMessageId = payload.clientMessageId;
      ...
      reactionSubRef.current = client.subscribe(reactionTopic, (message) => {
        try {
          ...
          // Confirmed from a real payload capture: sender identity is
          // `sender` (email), and `count` is present — almost certainly the
          // server's running total for this reaction type, not a delta.
          // Forwarding it through so the UI can set counts from the
          // server's number instead of incrementing locally, which is what
          // was causing creator and viewer to show different totals.
          const reaction: LiveReaction = {
            id: payload.id || Date.now(),
            session: payload.session || sessionId,
            reactionType: payload.reactionType || payload.type,
            user: payload.sender || payload.user,
            userId: payload.userId,
            timestamp: payload.timestamp || Date.now(),
          };
          (reaction as any).sender = payload.sender;
          (reaction as any).count = payload.count;
