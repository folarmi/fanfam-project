...
const LiveStreaming = () => {
  ...
  // Session id used to be a name the creator typed in manually. Backend
  // confirmed it isn't used for anything meaningful beyond being a unique
  // identifier — and reusing the same name across sessions causes a 500.
  // Generating a UUID up front removes both the naming step and the
  // collision risk entirely.
  const [channelName] = useState(() => uuidv4());
  const [_streamDuration, setStreamDuration] = useState(0);
  const [streamStartTime, setStreamStartTime] = useState<number | null>(null);
  const [streamDescription, setStreamDescription] = useState("");
  const [showTips, setShowTips] = useState(true);
  ...
  // Was `channelName || activeSession` — before the UUID change, channelName
  // was "" for viewers, so that fell back to activeSession (the real
  // session id from the URL) correctly. Now that channelName auto-generates
  // a UUID for every user regardless of role, it's always truthy, so a
  // viewer was fetching a token scoped to their own random unrelated UUID
  // instead of the actual stream's session — which is exactly what
  // CAN_NOT_GET_GATEWAY_SERVER: invalid token means (token and channel
  // don't match). activeSession is already correctly role-aware
  // (isHost ? channelName : urlSessionId), so just using it directly here
  // is both correct and simpler.
  const { isLoading: isLoadingToken, refetch: fetchToken } = useGetData({
    url: `/agora/rtc-token?channel=${activeSession}&uid=${userObject?.usid}`,
    queryKey: ["GetAgoraRTCToken", activeSession, userObject?.usid],
    enabled: false,
  });
  ...
  useEffect(() => {
    ...
    // Confirmed with backend: the actual "stream ended" signal comes
    // through /user/queue/live-notify (event: CREATOR_ENDED_LIVE) — the
    // same destination used for "creator went live" — not the
    // /topic/live/{sessionId}/end broadcast this hook also listens on
    // (isStreamEnded), which the backend doesn't appear to actually use.
    // Checking both so this keeps working either way.
    // Matching on sessionId, not just creatorId — a stale end-flag from a
    // previous, already-finished stream by this same creator would
    // otherwise immediately (and wrongly) match here too. sessionId scopes
    // this to the exact stream currently being watched. Falling back to
    // creatorId-only matching only if sessionId wasn't captured for some
    // reason (shouldn't normally happen, but safer than silently never
    // matching).
    const remoteEndedForThisCreator =
      lastEndedCreator &&
      activeSession &&
      (lastEndedCreator.sessionId
        ? lastEndedCreator.sessionId === activeSession
        : urlCreatorId && lastEndedCreator.creatorId === urlCreatorId);

    // Temporary diagnostic — unconditional, so we can see on the next test
    // exactly what this effect is comparing every time it re-runs.
    console.log("🔎 [end-check]", {
      isHost,
      isStreamEnded,
      lastEndedCreator,
      urlCreatorId,
      remoteEndedForThisCreator,
    });

    if (isStreamEnded || remoteEndedForThisCreator) {
      console.log("🔎 [end-check] triggering handleStopLive in 2s");
      showInlineToast({
        type: "info",
        title: "The stream has ended",
      });
      setTimeout(() => {
        console.log("🔎 [end-check] calling handleStopLive now");
        handleStopLive();
      }, 2000);
      return;
    }
  ...
  const handleStartLive = async () => {
    ...
    try {
      ...
      // channelName is now an auto-generated UUID (see useState above), so
      // there's nothing left for the host to type or validate here.
  ...
  const handleStopLive = async () => {
    console.log("🔎 [handleStopLive] invoked, isHost:", isHost);
    // Previously every teardown step below shared one try/catch — if any
    // single call threw (a track already closed, leave() rejecting, etc.)
    // execution jumped straight to the catch block and skipped
    // setIsStreaming(false) and the video element cleanup entirely. That's
  ...
  if (!isStreaming && isHost) {
    return (
      <div className="min-h-screen bg-blue_900 flex flex-col lg:flex-row">
        <div className="flex-1 flex flex-col items-center justify-center p-4 lg:p-8">
          <div className="w-full max-w-3xl">
            <div className="flex items-center mb-6">
              <button
                onClick={handleGoBack}
                className="text-white_200 hover:text-primary transition-colors"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <Typography
                variant="subtitle2"
                className="text-white_200 uppercase pl-2 tracking-wide"
              >
                Go Live
              </Typography>
              ...
              <div className="ml-auto">
                {isConnected ? (
                  <span className="flex items-center gap-2 text-green_600 text-sm">
                    <div className="w-2 h-2 bg-green_100 rounded-full animate-pulse" />
                    Connected
                  </span>
                ) : (
                  <span className="flex items-center gap-2 text-orange_100 text-sm">
                    <div className="w-2 h-2 bg-orange_100 rounded-full animate-pulse" />
                    Connecting...
                  </span>
                )}
              </div>
            ...
            <div className="relative bg-grey_900 backdrop-blur rounded-2xl overflow-hidden aspect-video shadow-2xl border border-grey_800">
              <video
                ref={videoRef}
                autoPlay
                muted
              ...
              {!isCameraOn && (
                <div className="absolute inset-0 flex items-center justify-center bg-black_400/70">
                  <div className="bg-grey_800/60 backdrop-blur-md p-8 rounded-full">
                    <VideoOff className="w-16 h-16 text-grey_400" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              <button
                ...
                className={`backdrop-blur p-4 rounded-full transition-all shadow-lg ${
                  isMicOn
                    ? "bg-grey_800 hover:bg-grey_700"
                    : "bg-red_300 hover:bg-red_400"
                }`}
              >
                {isMicOn ? (
                  <Mic className="w-6 h-6 text-white_200" />
                ) : (
                  <MicOff className="w-6 h-6 text-white_200" />
                )}
              </button>
              ...
              {/* Disabled (and relabeled) while the socket is down, mirroring
                  ...
                  then silently doing nothing useful. */}
              <CustomButton
                className="text-xs w-fit px-8 !bg-primary hover:!bg-primaryTwo disabled:!bg-grey_700"
                onClick={handleStartLive}
                disabled={isLoadingToken || !isConnected}
                title={!isConnected ? "Waiting for connection..." : undefined}
              >
              ...
              >
                {isCameraOn ? (
                  <Video className="w-6 h-6 text-white_200" />
                ) : (
                  <VideoOff className="w-6 h-6 text-white_200" />
                )}
              </button>
            </div>
          </div>
        ...
        <div className="w-full lg:w-96 bg-grey_900 p-6 border-t lg:border-t-0 lg:border-l border-grey_800">
          <div className="mb-6">
            <Typography
              variant="subtitle2"
              className="text-white_200 mb-3 block"
            >
              Description
            </Typography>
            <textarea
              ...
              maxLength={500}
              className="w-full bg-grey_800 text-white_200 placeholder-grey_400 px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary resize-none h-32"
            />
            <div className="text-right text-grey_400 text-sm mt-1">
              {streamDescription.length}/500
            </div>
          </div>

          <div className="flex items-center justify-between bg-grey_800 border border-grey_700 px-4 py-3 rounded-lg">
            <p className="text-white_200 font-medium text-sm">
              Show tips collected to viewers
            </p>
            <button
              onClick={() => setShowTips(!showTips)}
              className={`relative w-12 h-6 rounded-full transition-all ${
                showTips ? "bg-primary" : "bg-grey_600"
              }`}
            >
              <div
                className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white_200 rounded-full transition-transform ${
                  showTips ? "translate-x-6" : ""
                }`}
              />
            </button>
