...
const KycVerification = () => {
  ...
  // Protects against double-clicks/resubmits within this page load.
  // Does NOT survive a reload — a real fix needs backend to expose a
  // "kycSessionPendingAt" timestamp or equivalent on the profile object.
  ...
  const handleStartVerification = () => {
    if (justSubmitted || isVerified || isStartingDiditKyc) return;
  ...
  // Show loader while redirect to Edit Profile is in flight.
  if (!isProfileComplete) {
    return (
      <div className="flex min-h-64 items-center justify-center">
        <Loader2 className="h-7 w-7 animate-spin text-primary" />
