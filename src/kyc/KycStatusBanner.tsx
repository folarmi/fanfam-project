...
// components/kyc/KycStatusBanner.tsx
import { useNavigate } from "react-router";
import { ShieldCheck, Clock, XCircle, RotateCcw } from "lucide-react";
import { useKycSession } from "@/hooks/useKycSession";
...
export const KycStatusBanner = ({
  ...
  const handleCheckStatus = async () => {
    ...
    const refreshedSession = result.data;

    if (refreshedSession?.status === "APPROVED" && refreshedSession.sessionId) {
    ...
    return (
      <div className="flex items-start justify-between gap-4 rounded-xl border border-yellow-200 bg-yellow-50 p-4">
        ...
        <button
          ...
          disabled={isFetching || verifyCreatorMutation.isPending}
          className="shrink-0 rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-medium text-yellow-700 hover:bg-yellow-200 disabled:opacity-60"
        >
          {isFetching || verifyCreatorMutation.isPending
            ? "Checking..."
            : "Check status"}
        </button>
      </div>
  ...
  // Declined — failed the check, user can re-check status (it may have
  // been re-reviewed and approved) or start a fresh attempt
  if (status === "DECLINED") {
    return (
      <div className="flex items-start justify-between gap-4 rounded-xl border border-red-200 bg-red-50 p-4">
        <div className="flex items-start gap-3">
  ...
  // Expired / Kyc Expired / Abandoned — session lapsed or was never finished, needs a restart
  if (
    status === "EXPIRED" ||
    status === "KYC_EXPIRED" ||
    status === "ABANDONED"
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-orange-700">
              Your previous verification attempt is no longer valid. Please
              start a new one.
            </p>
          </div>
        </div>
  ...
  // Not Started, In Progress, Awaiting User, Not Finished, no session yet,
  // or any other status — default CTA banner
  return (
    <div className="flex items-start justify-between gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-primary">
