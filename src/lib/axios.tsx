...
//     if (token) {
//       console.log(`🚀 [Axios Request] Attaching token to ${config.url}:`, token.substring(0, 20) + "...");
...
//     if (status === 401) {
//         console.error(`❌ [Axios Response] 401 Unauthorized for ${originalRequest?.url}`, error.response?.data);
...
api.interceptors.response.use(
  ...
  async (error: AxiosError<any>) => {
    ...
    try {
      ...
      // Remove leading slash to prevent double slash issue
