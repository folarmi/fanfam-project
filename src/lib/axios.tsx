...
api.interceptors.response.use(
  ...
  async (error: AxiosError<any>) => {
    ...
    if (status === 401) {
      console.error(
        `❌ [Axios Response] 401 Unauthorized for ${originalRequest?.url}`,
      );
    ...
    const isAuthFailure = (status === 401 && isTokenExpired) || isMissingAuth;
    ...
    if (isRefreshing) {
      console.log(
        `⏳ [Token Refresh] Queueing request for ${originalRequest.url}`,
      );
    ...
    try {
      ...
      if (!newToken) {
        console.error(
          "❌ [Token Refresh] No access token in refresh response:",
          data,
        );
        throw new Error(
          "Failed to retrieve new access token from refresh response",
        );
      }
      ...
      console.log(
        `✅ [Token Refresh] Notified ${refreshSubscribers.length} queued requests`,
      );

      // Retry original request with new token
      originalRequest.headers.Authorization = `Bearer ${newToken}`;
      console.log(
        `🔄 [Token Refresh] Retrying original request to ${originalRequest.url}`,
      );
      ...
      if (refreshStatus === 401 || refreshStatus === 403) {
        console.warn(
          "🧹 [Token Refresh] Clearing tokens due to invalid refresh token",
        );
        ...
        console.error(
          "⚠️ [Token Refresh] Server error (500) - tokens NOT cleared. This might be temporary.",
        );
      } else {
        console.error(
          "⚠️ [Token Refresh] Network or other error - tokens NOT cleared",
        );
      }
