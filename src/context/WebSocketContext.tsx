...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({
  ...
  // Tracks the most recent "creator ended live" signal so any component
  // watching a specific creator's stream can react to it. Confirmed by
  // backend: this comes through the SAME /user/queue/live-notify
  // destination as "creator went live" — not a separate /topic/.../end
  // broadcast, which is why that never worked. Only the event field
  // differs: CREATOR_LIVE_STARTED vs CREATOR_ENDED_LIVE.
  const [lastEndedCreator, setLastEndedCreator] = useState<{
    creatorId: string;
    sessionId: string;
    timestamp: number;
  } | null>(null);
  ...
  const handleLiveNotification = useCallback((payload: LiveNotification) => {
    const rawPayload = payload as any;
    // Confirmed from a real payload capture: the backend sends `creatorID`
    // (capital ID), not `creatorId`.
    const creatorId = rawPayload?.creatorID || rawPayload?.creatorId;
    if (!creatorId) return;

    const event = rawPayload?.event;

    if (event === "CREATOR_ENDED_LIVE") {
      setLiveCreators((prev) => {
        if (!prev.has(creatorId)) return prev;
        const updated = new Map(prev);
        updated.delete(creatorId);
      ...
      // Cleared so a future CREATOR_LIVE_STARTED for this creator shows
      // the toast again instead of being treated as already-notified.
      notifiedCreatorsRef.current.delete(creatorId);
      // Storing sessionId alongside creatorId: without it, this flag is
      // only scoped to "this creator ended a stream at some point," which
      // never expires. A viewer joining a BRAND NEW stream from the same
      // creator later would immediately (and wrongly) match on creatorId
      // alone, even though the end event was for a completely different,
      // already-finished stream. Consumers should compare against the
      // current session id, not just the creator.
      setLastEndedCreator({
        creatorId,
        sessionId: rawPayload.sessionId || rawPayload.session || "",
        timestamp: Date.now(),
      });
      return;
    }

    // Anything else (including the expected CREATOR_LIVE_STARTED, and
    // defensively any unrecognized event) is treated as "went live" —
  ...
  const value: WebSocketContextType & {
    lastEndedCreator: typeof lastEndedCreator;
  } = {
    isConnected,
    client,
    liveCreators,
